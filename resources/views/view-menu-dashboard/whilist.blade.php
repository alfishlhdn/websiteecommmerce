@extends('admin.main')
@section('judul', 'Wishlist Pelanggan | AGA IT COMPUTER | Toko Komputer & Service Laptop Singosari Malang')

@section('content')
    <div class="p-6 bg-gray-100 min-h-screen">
        <h1 class="text-3xl font-bold text-gray-900 mb-6">Wishlist Pelanggan</h1>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 text-sm">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse ($wishlists as $wishlist)
                            <tr class="hover:bg-gray-50 transition-colors duration-150">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                                    {{ $wishlist->user->name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $wishlist->product->nama_produk }}</td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">
                                    {{ $wishlist->created_at->translatedFormat('d F Y') }}</td>
                                {{-- <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                            <button onclick="openDeleteModal({{ $wishlist->id }})" title="Hapus"
                        ...
                        @empty
                            <tr>
                                <td colspan="5" class="px-6 py-4 text-center text-sm text-gray-500">Wishlist kosong.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
    ...
    <div id="deleteModal"
        ...
        <div
            ...
            <div class="flex flex-col items-center text-center">
                ...
                <h2 class="text-xl font-bold text-gray-900 mb-2">Hapus Item dari Wishlist?</h2>
                <p class="text-sm text-gray-600 mb-6">
                    Apakah Anda yakin ingin menghapus item ini dari wishlist pelanggan? Tindakan ini tidak dapat dibatalkan.
                </p>
    ...
    <script>
        function openDeleteModal(wishlistId) {
            const modal = document.getElementById('deleteModal');
            const form = document.getElementById('deleteForm');
            form.action = `{{ url('whislist') }}/${wishlistId}`;
